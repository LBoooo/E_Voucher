...
import com.evoucher.accv.e_voucher.utils.DateUtils;
...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener{
    ...
    private Calendar adjustCalendarByLimit(Calendar c, int yearStart, int yearStop, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(yearGrego > yearStop){
                ...
                int daySway = DateUtils.getSumOfDayInMonthForGregorianByMonth(yearStop, MONTH_STOP_GREGORIAN);
            ...
            if(Math.abs(yearGrego - yearStart) < Math.abs(yearGrego - yearStop)){
                ...
                int daySway = DateUtils.getSumOfDayInMonthForLunarByMonthLunar(yearStop, MONTH_STOP_LUNAR_NORMAL);
    ...
    private void initValuesForY(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            Log.d("GregorianLunarCalendarV", "mYearPickerView.getMaxValue():" + mYearPickerView.getMaxValue());
//            mYearPickerView.setMaxValue(yearSway);
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim){
        ...
        if(isGregorian){
            ...
            int monthLeap = DateUtils.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if(monthLeap == 0){
                ...
                monthSway = DateUtils.convertMonthLunarToMonthSway(cc.get(ChineseCalendar.CHINESE_MONTH), monthLeap);
                newDisplayedVales = DateUtils.getLunarMonthsNamesWithLeap(monthLeap);
            }
        }
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            int dayStop = DateUtils.getSumOfDayInMonthForGregorianByMonth(cc.get(Calendar.YEAR), cc.get(Calendar.MONTH) + 1);
            ...
            int dayStop = DateUtils.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            int oldDayStop = DateUtils.getSumOfDayInMonth(oldYearFix, oldMonthSway, true);
            int newDayStop = DateUtils.getSumOfDayInMonth(newYearFix, newMonthSway, true);
            ...
            int newYearFixMonthLeap = DateUtils.getMonthLeapByYear(newYearFix);//1.计算当前year是否有闰月
            int oldYearFixMonthLeap = DateUtils.getMonthLeapByYear(oldYearFix);//2.计算之前year是否有闰月

            if(newYearFixMonthLeap == oldYearFixMonthLeap){
                //only update day picker
                newMonthSway = oldMonthSway;

                int oldMonthLunar = DateUtils.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int newMonthLunar = DateUtils.convertMonthSwayToMonthLunar(newMonthSway, newYearFixMonthLeap);
                int oldDayStop = DateUtils.getSumOfDayInMonthForLunarByMonthLunar(oldYearFix, oldMonthLunar);
                int newDayStop = DateUtils.getSumOfDayInMonthForLunarByMonthLunar(newYearFix, newMonthLunar);
                ...
                //月视图需要更新
                mCurrDisplayMonthsLunar = DateUtils.getLunarMonthsNamesWithLeap(newYearFixMonthLeap);

                //优化方案
                int oldMonthLunar = DateUtils.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int oldMonthLunarAbs = Math.abs(oldMonthLunar);
                newMonthSway = DateUtils.convertMonthLunarToMonthSway(oldMonthLunarAbs, newYearFixMonthLeap);
                ...
                //日视图需要更新
                int oldDayStop = DateUtils.getSumOfDayInMonth(oldYearFix, oldMonthSway, false);
                int newDayStop = DateUtils.getSumOfDayInMonth(newYearFix, newMonthSway, false);
                if(oldDayStop == newDayStop){
                    if(mOnDateChangedListener != null){
                        mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, oldDaySway, isGregorian));
                    }
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian){
        ...
        int oldDayStop = DateUtils.getSumOfDayInMonth(oldYear, oldMonth, isGregorian);
        int newDayStop = DateUtils.getSumOfDayInMonth(newYear, newMonth, isGregorian);
    ...
    public static class CalendarData{
        ...
        private void initChineseCalendar(){
            if(isGregorian){
                ...
                int y = pickedYear;
                int m = DateUtils.convertMonthSwayToMonthLunarByYear(pickedMonthSway, pickedYear);
                int d = pickedDay;
